          ...
          <section class="card">
            ...
            <div class="sensor-grid">
              ...
              <div class="sensor-tile">
                ...
                <span class="sensor-rgb">{{ colorEstado(ultima?.sensor) }}</span>
                <span class="sensor-lbl">Color detectado</span>
                <span class="sensor-tech num">{{ rgbText(ultima?.sensor) }}</span>
              </div>
            </div>
          </section>

          <section class="card decision-card">
            <h3 class="card-title">Ultima decision</h3>
            <div class="verdict" :class="'v--' + decisionActual(ultima?.clasificacion).tipo">
            ...
            <template v-if="ultima">
              ...
              <div class="foto-thumbs">
                <img v-for="(c, i) in capturas" :key="c.key" :src="c.url"
                     class="foto-thumb" :class="{ 'foto-thumb--sel': i === indiceSeleccionado }"
                     :title="'Foto ' + (i + 1)" :alt="'Foto ' + (i + 1)"
                     @click="indiceSeleccionado = i" />
...
.card3--total { background:#263241; }
.card3--sana  { background:#2f7d32; }
.card3--enf   { background:#c94732; }
.card3--exp   { background:#68727d; }
...
/* Sensores */
.sensor-grid { display:grid; grid-template-columns:repeat(3,1fr); gap:8px; padding:0 16px; }
.sensor-tile { background:var(--fondo-soft); border:1px solid var(--borde-cards); border-radius:8px; padding:12px 8px; display:flex; flex-direction:column; align-items:center; gap:5px; }
.sensor-val { font-size:2rem; font-weight:900; }
.sensor-swatch { width:44px; height:24px; border-radius:6px; border:1px solid var(--borde-cards); }
.sensor-rgb { font-size:.95rem; font-weight:900; color:var(--texto-primary); text-align:center; }
.sensor-tech { font-size:.68rem; font-weight:700; color:var(--texto-secondary); }
.sensor-lbl { font-size:.72rem; color:var(--texto-secondary); text-align:center; display:inline-flex; align-items:center; justify-content:center; gap:5px; }
...
/* Última fruta */
.col:first-child .card:last-child { flex:1; display:flex; flex-direction:column; }
.col:first-child .card:last-child .pie-wrap { flex:1; align-content:center; }
