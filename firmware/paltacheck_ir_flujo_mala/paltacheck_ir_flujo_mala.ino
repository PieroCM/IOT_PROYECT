/* ============================================================================
   PaltaCheck — FLUJO con IR + SENSORES + FOTO + SERVO   (palta MALA)
   ESP32-S3 N16R8 · script independiente
   ----------------------------------------------------------------------------
   Igual que paltacheck_ir_flujo, pero la palta se marca como MALA (antracnosis)
   y se usa el SERVO (compuerta) para rechazarla.

   Flujo (mientras haya lote activo):
     1) Cinta avanza HASTA que el IR detecta la fruta -> para -> 3 s.
     2) RODILLOS x3: giran -> 5 s -> FOTO ACTUAL -> manda al dashboard -> 5 s.
        (la foto se toma fresca: descarta frames viejos del buffer)
     3) Palta MALA -> el SERVO abre la compuerta para rechazarla.
     4) Expulsa al MAXIMO (ambos rodillos, mismo sentido) -> compuerta vuelve a 0.
     5) Si se cierra el lote en cualquier momento, FRENA todo.

   Requisito: librería ESP32Servo. Servo en GPIO 3, alimentado con 5V aparte.
   ============================================================================ */
...
void setup() {
  Serial.begin(115200);
  delay(300);
  Serial.println("\n===== FLUJO IR + Sensores + Foto + SERVO (palta MALA) =====");
...
// ── Duerme 'ms' vigilando el lote. false si se cierra. ──────────────────────
...
// ── Procesa la fruta MALA: rodillos x3 + foto/sensores -> servo -> bote ─────
void procesarPalta() {
  Serial.println("\n=== Procesando fruta (MALA) ===");
  ...
  for (int i = 1; i <= cantidadVueltas; i++) {
    ...
    int paltaId = enviarPalta(r,g,b,lux,t,h);   // MALA (antracnosis)
  ...
  // MALA -> SERVO abre la compuerta para rechazar
  Serial.println("\n[SERVO] palta MALA -> ABRE compuerta (rechazo)");
  moverCompuerta(ANGULO_COMPUERTA);
  delay(800);

  // Expulsar al MAXIMO (ambos rodillos, mismo sentido)
  Serial.println("[Botar] expulsando al MAXIMO (ambos mismo sentido)");
  if (!botarPalta(tiempoBotar)) { abortarPorLoteCerrado(); return; }
  detenerTodo();

  // Cerrar compuerta para la siguiente
  moverCompuerta(0);
}
...
void rodillosParaBotarPalta(int v1, int v2) {
  // Ambos en el MISMO sentido a máxima potencia
  digitalWrite(IN1_RODILLO1, LOW);  digitalWrite(IN2_RODILLO1, HIGH); ledcWrite(ENA_RODILLO1, v1);
  digitalWrite(IN3_RODILLO2, LOW);  digitalWrite(IN4_RODILLO2, HIGH); ledcWrite(ENB_RODILLO2, v2);
}
...
// POST /api/palta  (MALA = "antracnosis").
int enviarPalta(int r, int g, int b, float lux, float t, float hr) {
  ...
  snprintf(body, sizeof(body),
    "{\"lote_id\":%d,\"clasificacion\":\"antracnosis\",\"confianza\":1.0,"
    "\"r\":%s,\"g\":%s,\"b\":%s,\"lux\":%s,"
  ...
  if (code == 200) {
    ...
    Serial.printf("  [NET] POST /api/palta OK (antracnosis) palta_id=%d\n", paltaId);
  } else {
